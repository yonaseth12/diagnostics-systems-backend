...
# Common asset path
MJ_ASSET_PATH = 'assets/city_mj_assets'

# Loading label encoder, scaler, and model from assets
with open(os.path.join(MJ_ASSET_PATH, 'label_encoder_mj.pkl'), 'rb') as f:
    label_encoder_mj = pickle.load(f)
with open(os.path.join(MJ_ASSET_PATH, 'scaler_mj.pkl'), "rb") as f:
    scaler_mj = pickle.load(f)

# Loading trained model and projectors
model = DiseaseClassifierSoftmax(num_classes=len(label_encoder_mj.classes_))
model.load_state_dict(torch.load(os.path.join(MJ_ASSET_PATH, "disease_classifier_mj.pth")))
model.eval()
...
def predict_disease_mj(note: str, age: float, gender: str, body_temp: float, systolic: float, diastolic: float):
    predictions = predict_disease_mj_func(note, age, gender, body_temp, systolic, diastolic)
...
def predict_disease_mj_func(note: str, age: float, gender: str, body_temp: float, systolic: float, diastolic: float):
    ...
    X_struct_scaled = scale_structured_data_inference(X_struct, scaler_mj)
    ...
    predictions = dict(zip(label_encoder_mj.classes_, probs))
...
# Example usage
# predictions = predict_disease_mj(
